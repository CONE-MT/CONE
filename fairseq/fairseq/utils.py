...
LOW_RESOURCES_ZH = [
    "ss", "crh", "gv", "or", "as", "kab", "syr", "chr", "rom", "dop", "cop", "ee", "cjp", "quw", "shi", "quc", "usp", "djk",
    "amu", "jak", "tn", "cak", "nhg", "chq", "mam", "ppk", "cni", "wal", "gbi", "li", "ch", "agr", "ojb", "dik", "ake",
    "jiv", "gd", "acu", "bsn"]
...
FAMILY_7 = {
    ...
    "family_2": ["sv", "nl", "de"],
    "family_3": ["zh", "ja"],
    "family_4": ["sr", "pl", "cs", "ru", "vi"],
    "family_5": ["fi", "hu", "el"],
    "family_6": ["es", "fr", "pt", "it", "ro"],
    "family_7": ["bg", "en"],
    "family_8": ["fi", "da"],
    "low_resource": LOW_RESOURCES_ZH
}

FAMILY_3 = {
    "family_1": ["tr", "ar", "ja", "he", "zh"],
    "family_2": ["ro", "it", "es", "pt", "fr", "hu", "el"],
    "family_3": ["en", "bg", "cs", "sr", "pl", "ru", "vi", "id", "sv", "de", "nl"],
    "family_4": ["fi", "da"],
    "low_resource": LOW_RESOURCES_ZH
}
...
def get_family(family_type):
    if family_type is None or family_type.lower() == "original":
        return FAMILY_DEFAULT
    if family_type.lower() == "family_7":
        return FAMILY_7
    elif family_type.lower() == "family_3":
        return FAMILY_3
    elif family_type.lower() == "english_centered_family":
        from cm2m_utils.english_centered_family_info import ENGLISH_CENTRAL_FAMILY
        return ENGLISH_CENTRAL_FAMILY
    elif family_type.lower() == "x_x_family_8":
        from cm2m_utils.english_centered_family_info import X_X_FAMILY_8
        return X_X_FAMILY_8
    else:
        raise ValueError("unknown family info: %s" % family_type)


def get_lg_family_info(lang, family_type):
    FAMILY = get_family(family_type)
    for f_k, f_v in FAMILY.items():
        if lang in f_v:
            return f_k
    return lang


def get_lg_family_pair(lang_pair, family_type):
    src, tgt = lang_pair.split("-")
    src_family_info, tgt_family_info = get_lg_family_info(src, family_type), get_lg_family_info(tgt, family_type)
    return "%s-%s" % (src_family_info, tgt_family_info)


def reset_model_state_dict_(model, lg_pair=None, core_lang=None, keep_main=False):

    def _reset_one_branch(lg_pair):
        value = None
        for key in model.keys:
            if lg_pair == "main-m2m" and key == "main-m2m":
                value = model.models[key]
            elif lg_pair.startswith("main") and key.startswith("main"):
                value = model.models[key]
            elif lg_pair.endswith("m2m") and key.endswith("m2m"):
                value = model.models[key]
        if value is None:
            value = model.models[model.keys[0]]
        # assert value is not None
        return {lg_pair: value}

    def _reset_core_lang_branches(core_lang):
        values = {}
        for key in model.keys:
            if key == "main-m2m":
                if keep_main:
                    values[key] = model.models[key]
                continue
            if key.startswith("main"):
                values["main-%s" % core_lang] = model.models[key]
            elif key.endswith("m2m"):
                values["%s-m2m" % core_lang] = model.models[key]
        return values

    if lg_pair is None and core_lang is None:
        return model

    new_model_state = {}
    if lg_pair is not None:
        new_model_state = _reset_one_branch(lg_pair)

    if core_lang is not None:
        new_model_state = _reset_core_lang_branches(core_lang)

    tmp_models = nn.ModuleDict(new_model_state)
    src = list(set([key.split("-")[0] for key in new_model_state.keys()]))
    tgt = list(set([key.split("-")[1] for key in new_model_state.keys()]))
    model.encoder_keys, model.decoder_keys, model.keys = src, tgt, list(new_model_state.keys())
    model.models = tmp_models
    if hasattr(model, "module"):
        model.module.module.models = tmp_models
        model.module.module.encoder_keys, model.module.module.decoder_keys, model.module.module.keys \
            = src, tgt, list(new_model_state.keys())
    return model
...
class FileContentsAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        if PathManager.isfile(values):
            if "s3://" in values:
                argument = PathManager.get_ceph_manager().get(values).decode("utf-8").strip()
                logger.info("argument is: %s " % argument)
            else:
...
def split_paths(paths: str, separator=os.pathsep) -> List[str]:
    if "s3://" in paths:
        return (paths.split("@@")) if "@@" in paths else paths.split(MANIFOLD_PATH_SEP)
    else:
        return (
...
def load_ensemble_for_inference(filenames, task, model_arg_overrides=None):
    ...
    deprecation_warning(
        "cm2m_utils.load_ensemble_for_inference is deprecated. "
