...
def find_offsets_ceph_path(url: str, num_chunks: int) -> tp.List[int]:
    client = PathManager.get_ceph_manager()
    size = client.size(url)

    def _safe_read_chuck(client, url, start, end):
        line = client.get_with_range(url, range="%s-%s" % (start, end))
        tmp_end = 0
        while True:
            tmp_str = line[: tmp_end]
            if tmp_str.endswith(b"\n"):
                return tmp_end+start - 1
            else:
                tmp_end = tmp_end + 1
    ...
    for i in range(1, num_chunks):
        start = chunk_size * i
        end = chunk_size * i + 1024
        offsets[i] = _safe_read_chuck(client, url, start, end)
    offsets[-1] = size
    return offsets
