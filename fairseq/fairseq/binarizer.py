...
from fairseq.file_chunker_utils import Chunker, find_offsets,find_offsets_ceph_path
from fairseq.file_io import PathManager, _is_use_ceph
...
class FileBinarizer:
    ...
    def multiprocess_dataset(
        ...
        offsets = find_offsets_ceph_path(input_file, num_workers) if _is_use_ceph(input_file) else find_offsets(input_file, num_workers)
        ...
        if num_workers > 1:
            for worker_id in range(1, num_workers):
                ...
                try:
                    PathManager.rm(indexed_dataset.data_file_path(worker_output_prefix))
                    PathManager.rm(indexed_dataset.index_file_path(worker_output_prefix))
    ...
    def _binarize_file_chunk(
        ...
        def ceph_binarize():
            client = PathManager.get_ceph_manager()
            chunck = client.get_with_range(filename, range="%s-%s" % (offset_start, offset_end)).decode("utf-8")
            for line in chunck.split("\n"):
                if len(line) == 0:
                    continue
                ds.add_item(binarizer.binarize_line(line, summary))

        ceph_binarize() if _is_use_ceph(filename) else local_binarize()
        if do_save and _is_use_ceph(filename) : ds.ceph_write()
        return ds, summary
...
class AlignmentDatasetBinarizer(Binarizer):
    ...
    them in a tensor (see cm2m_utils.parse_alignment)
    """
