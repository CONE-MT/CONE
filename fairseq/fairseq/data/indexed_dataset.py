...
try:
 ceph_util = CEPHFileUtil()
    ...
    logging.warning("no ceph manager")
...
def infer_dataset_impl(path):
    if IndexedRawTextDataset.exists(path):
        ...
        try:
            ...
            f = io.BytesIO(ceph_util.get(index_path)) if "s3://" in index_path else open(index_path, "rb")
            magic = f.read(8)
...
def _warmup_mmap_file(path):
    stream = None
    try:
        stream = io.BytesIO(ceph_util.get(path)) if "s3://" in path else open(path, "rb")
...
class MMapIndexedDataset(torch.utils.data.Dataset):
    class Index:
        ...
        def writer(cls, path, dtype):
            class _Writer:
                def __enter__(self):
                    self._file = io.BytesIO() if "s3://" in path else open(path, "wb")
                    self.path = path
                ...
                def __exit__(self, exc_type, exc_val, exc_tb):
                    if "s3://" in self.path:
                        PathManager.get_ceph_manager().write(self.path, self._file.getvalue())
                    self._file.close()
        ...
        def __init__(self, path):
            ...
            if "s3://" in path:
                self._bin_buffer_mmap = ceph_util.get(path)
            else:
        ...
        def __del__(self):
            if hasattr(self._bin_buffer_mmap, "_mmap"):
    ...
    def _do_init(self, path):
        ...
        if "s3://" in self._path:
            self._bin_buffer_mmap = ceph_util.get(data_file_path(self._path))
        else:
...
class MMapIndexedDatasetBuilder:
    def __init__(self, out_file, dtype=np.int64):
        self.is_ceph_path = "s3://" in out_file
        self._data_file = io.BytesIO() if self.is_ceph_path else open(out_file, "wb")
    ...
    def ceph_write(self):
        PathManager.get_ceph_manager().write(self.out_file, self._data_file.getvalue())
    ...
    def merge_file_(self, another_file):
        ...
        # Concatenate data
        def local_concatenate():
            with open(data_file_path(another_file), "rb") as f:
                shutil.copyfileobj(f, self._data_file)

        def ceph_concatenate():
            with io.BytesIO(ceph_util.get(data_file_path(another_file))) as f:
                self._data_file.write(f.getvalue())

        ceph_concatenate() if "s3://" in another_file else local_concatenate()

    def finalize(self, index_file):
        if self.is_ceph_path:
            self.ceph_write()
