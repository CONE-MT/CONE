...
class StreamingEpochBatchIterator(EpochBatchIterating):
    """A steaming-style iterator over a :class:`torch.cm2m_utils.data.IterableDataset`.

    Args:
        dataset (~torch.cm2m_utils.data.Dataset): dataset from which to load the data
        ...
        buffer_size (int, optional): the number of batches to keep ready in the
            ...
            default torch.cm2m_utils.data.DataLoader preloading is used.
...
class EpochBatchIterator(EpochBatchIterating):
    """A multi-epoch iterator over a :class:`torch.cm2m_utils.data.Dataset`.

    Compared to :class:`torch.cm2m_utils.data.DataLoader`, this iterator:
    ...
    Args:
        dataset (~torch.cm2m_utils.data.Dataset): dataset from which to load the data
        collate_fn (callable): merges a list of samples to form a mini-batch
        batch_sampler (~torch.cm2m_utils.data.Sampler or a callable): an iterator over batches of
            indices, or a callable to create such an iterator (~torch.cm2m_utils.data.Sampler).
