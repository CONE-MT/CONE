...
class FairseqMultiModel(BaseFairseqModel):
    ...
    def __init__(self, encoders, decoders, lang_pairs=None, family_type=None):
        ...
        self.decoder_keys = list(decoders.keys())
        for encoder_key, decoder_key in zip(self.encoder_keys, self.decoder_keys):
            check_type(encoders[encoder_key], FairseqEncoder)
            check_type(decoders[decoder_key], FairseqDecoder)
        self.keys = self.encoder_keys if lang_pairs is None else lang_pairs
        if lang_pairs is None:
            self.models = nn.ModuleDict(
                {
                    key_pair: FairseqEncoderDecoderModel(encoders[key_pair], decoders[key_pair])
                    for key_pair in self.encoder_keys
                }
            )
        else:
            self.models = nn.ModuleDict(
                {
                    key_pair: FairseqEncoderDecoderModel(encoders[utils.get_lg_family_info(key_pair.split("-")[0], family_type)],
                                                         decoders[utils.get_lg_family_info(key_pair.split("-")[1], family_type)])
                    for key_pair in lang_pairs
                }
            )
