...
def average_checkpoints(inputs):
    ...
    for fpath in inputs:
        state = ceph_util.load_checkpoint(fpath, torch.device("cpu"))
...
def main():
    ...
    is_s3_path = True if "s3://" in args.output else False
    if is_s3_path:
        with io.BytesIO() as stream:
            torch.save(new_state, stream)
            stream.seek(0)
            PathManager.get_ceph_manager().write(args.output, stream)
    else:
