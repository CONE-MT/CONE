...
def main(args):
    ...
    if not args.srcdict and PathManager.exists(_dict_path(args.source_lang, args.destdir)):
        raise FileExistsError(_dict_path(args.source_lang, args.destdir))

    if (
        target
        and not args.tgtdict
        and PathManager.exists(_dict_path(args.target_lang, args.destdir))
    ):
